...
for item in RAW_OFFICER_LIST:
    ...
    OFFICER_DATABASE[name] = [buyou, chiryaku, tousotsu, skill_name, 100, 120, "受動", s_attr, base_aptitudes, traits]

OFFICER_LIST = sorted(list(OFFICER_DATABASE.keys()))

def get_officer_data(o_name):
    return OFFICER_DATABASE.get(o_name, [100, 100, 100, "汎用", 50, 100, "能動", "兵刃", {"足軽": 1, "騎兵": 1, "弓兵": 1, "鉄砲": 1}, []])
...
def input_team_data(team_prefix, team_name, default_choices, default_troop_idx):
    ...
    for idx, role in enumerate(roles):
        ...
        with st.expander(f"【{role}】 {default_choices[idx]}", expanded=(idx==0)):
            ...
            # 簡易ステータス表示
            st.caption(f"武勇: **{o_buyou}** | 知略: **{o_chiryaku}** | 統率: **{o_tousotsu}** | 固有戦法: **{db_s1_name}**")

            # 特性（凸連動）のコンパクト表示
            trait_texts = []
            for t in traits:
                icon = "✅" if rank >= t["req"] else "🔒"
                trait_texts.append(f"{icon} {t['req']}凸:{t['name']}")
            st.markdown(" ".join(trait_texts))
            ...
            team_officers.append({
                ...
                "buyou": o_buyou, "chiryaku": o_chiryaku, "tousotsu": o_tousotsu,
                "skills": skills
            })
